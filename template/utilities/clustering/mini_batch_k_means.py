...
from . import clustering_eval_viz
...
def execute_mini_batch_k_means(
        ...
        save_results_to_excel=False, export_charts=False, palette=None,
        **kwargs):
    """
    ...
    if standardize_vars:
        dataset, scaler = clustering_utilities.standardize_variables(data[variables])
    else:
    ...
    if generate_charts:
        ...
        cluster_plot, plot_info = clustering_eval_viz.clustering_scatter_plot(variables, dataset, new_variable_name)

        # create factor plot
        factor_plot = clustering_eval_viz.factor_plot(dataset, new_variable_name, palette)
    ...
    cluster_n = clustering_eval_viz.num_clusters_df(dataset, new_variable_name)
    scores = clustering_eval_viz.score_all(dataset.drop(columns=[new_variable_name]), prediction)
...
def mini_batch_k_means_range(
        data, variables, min_clusters=2, max_clusters=5,
        output_path=Path.cwd() / "outputs",
        standardize_vars=False, generate_charts=True, save_results_to_excel=False, export_charts=False, palette = None,
        **kwargs
):
    """
    ...
    for num_clusters in range(min_clusters, max_clusters + 1):
        ...
        k_means_model = execute_mini_batch_k_means(
