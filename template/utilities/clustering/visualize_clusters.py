...
def slice_1D(frame, x, cluster = None, max_points = 1000, style = "scatter"):
    ...
    if style == 'scatter':
        ...
        for c_idx, cluster in enumerate(unique_clusters):
            ...
            plt.scatter(c_cluster.iloc[:, 0], c_cluster["One"], label='Cluster {}'.format(c_idx))  #set y coordinate as 0
    ...
    elif large_style == "hex":
        plot = sns.jointplot(x=plot_data.reset_index().iloc[:, 1], y=plot_data.reset_index().iloc[:, 2],
                             kind='hex', stat_func=None)
        # Currently not able to extract data from jointplot, so using a sample dataset in return
    else:
        raise ValueError("The style selected '{}' for large scale plotting is not valid".format(large_style))
...
def slice_3D(frame, x, y, z, cluster = None, max_points = 1000, style = "scatter"):
    ...
    if style == "scatter":
        ...
        ax.scatter(plot_data.iloc[:, 0], plot_data.iloc[:, 1], plot_data.iloc[:, 2], c=plot_data.iloc[:, 3],
                   s=60)
    elif large_style == "density":
...
def plot_by_pc_by_cluster_top_two_dims(frame, target_prediction, other_cols_to_exclude=None):
    ...
    fig, ax, plot_data = slice_2D(frame_with_pc, "pc_dim_1", "pc_dim_2", cluster=target_prediction,
                               style="scatter")
...
def plot_by_pc_by_cluster_top_three_dims(frame, target_prediction, other_cols_to_exclude=None):
    ...
    pca_frame = pd.DataFrame(fitted_pca.transform(pca_to_fit_df),
                             columns=['pc_dim_{}'.format(ix+1) for ix in range(n_components)])
    ...
    return fig, plot_data, x_axis_label, y_axis_label, z_axis_label, plot_title, full_output_data, res, total_var_explained
...
def get_OW_palette():
    ...
    """
    owpalette = ['#002C77', '#9DE0ED', '#E29815', '#41A441', '#646EAC', '#DD712C', '#079B84', '#CB225B', '#008AB3', '#5F5F5F', '#DFDFDF', '#FFCF89', '#BDDDA3', '#C5CAE7', '#FDCFAC', '#A8DAC9', '#F8B8BC', '#BEBEBE']
...
def tool_plot(var_names, model_with_prediction, new_variable_name):
    """
        ...
        :return: dictionary of plot information for kmeans, plot object
    """
    temp_data_info = dict()
    ...
    if num_dim > 3:
        ...
        temp_data_info["axes_labels"] = {"x_axis_label":x_axis_label,"y_axis_label":y_axis_label,"z_axis_label":z_axis_label}
    ...
    # Call plotting based on number of variables
        ...
        if num_dim == 3:
            ...
            temp_data_info["axes_labels"] = {"x_axis_label": var_names[0], "y_axis_label": var_names[1], "z_axis_label": var_names[2]}
        elif num_dim == 2:
            my_plot, ax, plot_data = slice_2D(model_with_prediction, var_names[0], var_names[1],
                                            cluster=new_variable_name, max_points=1000, style="scatter")
            temp_data_info["axes_labels"] = {"x_axis_label": var_names[0], "y_axis_label": var_names[1]}
        else:
...
def get_factorplot(clusters_1, new_variable_name):
    """
        :param model_with_prediction: predicted model
        :param new_variable_name: name of column containing the predicted clusters
        Returns factorplot
        :return: g, containing the factor_plot
    """

    df_long = pd.melt(clusters_1, new_variable_name, var_name="Variable Names", value_name="Value")
    g = sns.factorplot("Variable Names", hue=new_variable_name, y="Value", data=df_long, kind="box", size=7, aspect=1.5,
                       palette=get_OW_palette(), legend=False)
    # g = sns.factorplot("a", hue=new_variable_name, y="Value", data=df_long, kind="point", size=7, aspect=1.5)
    plt.legend(loc='upper left')
    # kind options - {point, bar, count, box, violin, strip}
