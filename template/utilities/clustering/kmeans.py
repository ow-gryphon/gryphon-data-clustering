...
def execute_k_means(data: pd.DataFrame, variables: List[str], num_clusters: int,
    ...
    if standardize_vars:
        use_data, scaler = clustering_utilities.standardize_variables(data[variables])
    else:
    ...
    cluster_n = clustering_eval_viz.num_clusters_df(use_data, new_variable_name)

    # Set train metrics
    scores = clustering_eval_viz.score_all(use_data.drop(columns=new_variable_name), use_data[new_variable_name])
    ...
    if generate_charts:
        ...
        cluster_plot, plot_info = clustering_eval_viz.clustering_scatter_plot(
            variables,
        ...
        # create factor plot
        g = clustering_eval_viz.factor_plot(use_data, new_variable_name)
...
def k_means_range(dataset: pd.DataFrame, variables: List[str],
    ...
    if generate_charts:
        ...
        elbow_plot = clustering_eval_viz.elbow_plot(scores=pd.DataFrame(all_scores))
